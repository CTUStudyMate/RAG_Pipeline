...
```
Parse PDF:        O(pages) ~ milliseconds to seconds
Build Hierarchy:  O(toc_entries) ~ milliseconds
Compute Tokens:   O(nodes + atomics) ~ fast
Chunk (HSF):      O(atomics) ~ fast (single pass)
Generate Desc:    O(images × LLM_latency) ~ seconds per chunk
Index:            O(chunks × embedding_latency) ~ seconds to minutes

TOTAL: Dominated by LLM calls
  ├─ Without image descriptions: ~1-5 seconds per document
  └─ With image descriptions: +30-60 seconds per 100 images
```

### Space Complexity

```
SQLite DB:        O(atomics) ~ proportional to doc size
Hierarchy Tree:   O(depth × breadth) ~ small for most docs
Embeddings:       O(chunks × embedding_dim) ~ 100MB per 1000 chunks
PostgreSQL:       O(chunks + images) ~ searchable index

Total: Linear in document size, manageable for typical workflows
```
...
### How to Evaluate HSF Output

```
✓ GOOD CHUNK:
  ├─ Contains complete semantic unit
  ├─ Token count within [MIN, MAX]
  ├─ Headings/images included if relevant
  ├─ Sentence boundaries not broken
  └─ Related content grouped together

✗ BAD CHUNK:
  ├─ Incomplete sentence / orphaned text
  ├─ Exceeds max tokens
  ├─ Random cutting point
  ├─ Lost image reference
  └─ Unrelated content mixed

METRICS TO TRACK:
  Distribution of chunk sizes
    → Should cluster near middle of [MIN, MAX]
    
  Number of chunks exceeding MAX
    → Should be 0 (HSF guarantee)
    
  Average tokens per chunk
    → (min + max) / 2 is good target
...
## 10. Example: Full Walkthrough

### Input Document

```
Research Paper: "ML Optimization Techniques"
Total: 15 pages, 8000 tokens

Structure:
1. Abstract (200 tokens)
2. Introduction (800 tokens)
3. Related Work (1200 tokens)
4. Method (3000 tokens)
   4.1 Preliminaries (600)
   4.2 Algorithm A (1200)
   4.3 Algorithm B (1200)
5. Experiments (2000 tokens)
6. Conclusion (800 tokens)
```

### HSF Execution with MAX=600, MIN=200

```
Step 1: Build hierarchy
  ROOT
  ├─ Abstract [200]         ✓ Final: 1 chunk
  ├─ Introduction [800]     ✗ > 600, no children
  ├─ Related Work [1200]    ✗ > 600, no children
  ├─ Method [3000]          ✗ > 600, HAS children
  │  ├─ Preliminaries [600] ✓ Final: 1 chunk
  │  ├─ Algorithm A [1200]  ✗ > 600, split → 2 chunks
  │  └─ Algorithm B [1200]  ✗ > 600, split → 2 chunks
  ├─ Experiments [2000]     ✗ > 600, no children → split
  └─ Conclusion [800]       ✗ > 600, no children → split

Step 2: Chunking decisions
  ├─ Abstract → [1 chunk: 200]
  ├─ Intro → chunk_by_semantic_units → [2 chunks: 400 + 400]
  ├─ Related → chunk_by_semantic_units → [2 chunks: 600 + 600]
  ├─ Method.Preliminaries → [1 chunk: 600]
  ├─ Method.AlgA → chunk_by_semantic_units → [2 chunks: 600 + 600]
  ├─ Method.AlgB → chunk_by_semantic_units → [2 chunks: 600 + 600]
  ├─ Experiments → chunk_by_semantic_units → [3 chunks: 600 + 600 + 800?]
  │  └─ Last chunk 800 > 600, split again → [600 + 200]
  └─ Conclusion → chunk_by_semantic_units → [2 chunks: 400 + 400]

Final output: 17 chunks, all within [200, 600] tokens
  Average: ~470 tokens/chunk (good utilization)
```
...
| Aspect | HSF | Fixed-Size | Recursive Splitter |
|--------|-----|-----------|-------------------|
| **Preserves Structure** | ✓ Yes | ✗ No | Partial |
| **Semantic Boundaries** | ✓ Yes | ✗ No | Partial |
| **Token Compliance** | ✓ Guaranteed | ✓ Guaranteed | Heuristic |
| **Image Handling** | ✓ Native | ✗ Breaks | ✗ Breaks |
| **Large Outliers** | ✓ Handled | ✓ Handled | ✗ May exceed |
