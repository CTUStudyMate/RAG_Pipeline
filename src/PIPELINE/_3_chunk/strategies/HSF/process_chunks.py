...
def create_chunk(node, cursor, file_path):
    ...
    reconstructed_text = ""
    # base_section_text = {node["metadata"]["description"]}
    # reconstructed_text += f"[SECTION: {base_section_text}] "
    ...
    for atomic in atomics:
        ...
        if atomic["type"] == "text":
            ...
            if atomic["heading_type"]== "nottoc" or atomic["heading_type"]== "nottoc":
                ...
                text_content = atomic["content"]
                # if not is_heading_match(text_content, base_section_text):
                #     print("atomic description is")
                #     print(atomic["description"])
                if reconstructed_text == "":
                    reconstructed_text += f"[SECTION]: {atomic["description"]} "
                else:
                    reconstructed_text += f"\n\n[SECTION]: {atomic["description"]} "
        
        # elif atomic["type"] == "table":
        #     reconstructed_text += f"\n{atomic["content"]}"   
        
        # elif atomic["type"] == "list_item":
        #     reconstructed_text += f"\n - {atomic["content"]}" 
