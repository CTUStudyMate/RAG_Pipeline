...
def parse_pdf_into_atomic_units(batches, file_path, target_file, document_store_name, tree_file, conn, passed_cursor, open_node, node):
    ...
    for json_file in sorted(folder.glob("*.json")):
        ...
        # bắt đầu xem và sửa lại hàm handle batch result
            current_atomic_order, incomplete_atomic, passed_cursor, open_node, node = handle_batch_result(
                flat_list,
                dfs_clone,
                result,
                stream_elements,
                current_atomic_order,
                incomplete_atomic,
                target_file,
                dbcursor,
                document_store_name,
